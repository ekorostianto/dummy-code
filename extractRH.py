...
product_path = "/media/r140e/0AB80A130AB80A13/KP_2020/data/modis/myd07_l2"
input_files = pd.DataFrame(sorted(list(iglob(os.path.join(product_path, '**', '*.hdf'), recursive=True))))
with open('/media/r140e/0AB80A130AB80A13/KP_2020/data/modis/myd07_l2.txt', 'w') as f:
    f.write(
        input_files.to_string(header = False, index= False)
    )

try:
	fileList=open('/media/r140e/0AB80A130AB80A13/KP_2020/data/modis/myd07_l2.txt','r')
except:
...
for FILE_NAME in fileList:
	...
	if 'L2' in FILE_NAME:#Same as above but for 10km MODIS file
		userInput=int(1)
		dataFields=dict([(1,'Retrieved_Moisture_Profile'),(2,'Retrieved_Temperature_Profile')])
	...
	offset=int(attributes['add_offset'])
	#get SDS data
	data=sds.get()
    #input overlay point
    #suf7
	...
	if (user_lat < min_lat or user_lat > max_lat) or (user_lon < min_lon or user_lon > max_lon):
		...
		plevel=data[15]
		valid_data=((plevel[x,y])-offset) * scale_factor
		celcius=round(valid_data-273.15,2)
		if plevel[x,y]==fillvalue:
			print(' ')
